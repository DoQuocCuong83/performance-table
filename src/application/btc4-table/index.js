...
const Cell = (props) => {
  ...
  const value = useSelector((state) => {
    if (state.previousBTCs4[id]) {
      if (state.previousBTCs4[id][column] === state.currentBTCs4[id][column]) {
        cellChange.current = false;
      } else if (
        state.previousBTCs4[id][column] > state.currentBTCs4[id][column]
      ) {
    ...
    if (state.currentBTCs4[id]) {
      return state.currentBTCs4[id][column];
    }
  });
