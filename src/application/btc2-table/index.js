...
const Cell = (props) => {
  ...
  const value = useSelector((state) => {
    if (state.previousBTCs2[id]) {
      if (state.previousBTCs2[id][column] === state.currentBTCs2[id][column]) {
        cellChange.current = false;
      } else if (
        state.previousBTCs2[id][column] > state.currentBTCs2[id][column]
      ) {
    ...
    if (state.currentBTCs2[id]) {
      return state.currentBTCs2[id][column];
    }
  });
